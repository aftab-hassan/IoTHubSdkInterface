...
------------
	...
	async Task ReceiveC2DDesiredPropertyChangeAsync (DeviceInfo deviceInfo, DesiredPropertyUpdateCallback callback, , TransportType type)
...
---------------
	...
	2	Send message from device to cloud	async Task<Result>SendMessageD2CAsync(DeviceInfo deviceInfo, List<TelemetryData> data, TransportType type)	Send message(s) from device to cloud
		Input arguments
